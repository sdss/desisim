...
class TestObs(unittest.TestCase):
    ...
    def test_sample_nz(self):
        ...
        for objtype in ['LRG', 'ELG', 'QSO', 'STAR', 'STD']:
            z = lvmsim.targets.sample_nz(objtype, n)
            self.assertEqual(len(z), n)

    def test_get_targets(self):
        n = 5
        for flavor in ['DARK', 'BRIGHT', 'LRG', 'ELG', 'QSO', 'BGS', 'MWS']:
            fibermap, targets = lvmsim.targets.get_targets(n, flavor)
            fibermap, targets = lvmsim.targets.get_targets(n, flavor.lower())

    def test_sample_objtype(self):
        for flavor in ['DARK', 'GRAY', 'BRIGHT', 'LRG', 'ELG', 'QSO', 'BGS', 'MWS']:
            for n in [5,10,50]:
                for i in range(10):
                    truetype, targettype = lvmsim.targets.sample_objtype(n, flavor)

    def test_parallel(self):
        import multiprocessing as mp
        nproc = mp.cpu_count() // 2
        for n in [5, 23, 15*nproc+7]:
            fibermap, (flux, wave, meta) = lvmsim.targets.get_targets_parallel(n, 'DARK')
    ...
    def test_parallel_radec(self):
        ...
        nspec = 60
        fibermap, (flux, wave, meta) = lvmsim.targets.get_targets_parallel(nspec, 'SKY')
    ...
    def test_random(self):
        for nspec in (15, 30):
            fibermap1, (flux1, wave1, meta1) = lvmsim.targets.get_targets_parallel(nspec, 'DARK', seed=nspec+1)
            fibermap2a, (flux2a, wave2a, meta2a) = lvmsim.targets.get_targets_parallel(nspec, 'DARK', seed=nspec+2)
            fibermap2b, (flux2b, wave2b, meta2b) = lvmsim.targets.get_targets_parallel(nspec, 'DARK', seed=nspec+2)
