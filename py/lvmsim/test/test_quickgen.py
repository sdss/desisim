...
from lvmsim import obs
from lvmsim import simexp
from lvmsim.scripts import quickgen
...
class TestQuickgen(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.testEnv = dict(
            ...
            LVM_SPECTRO_SIM = os.path.join(cls.testDir,'spectro','sim'),
            LVM_SPECTRO_DATA = os.path.join(cls.testDir,'spectro','sim', 'test-quickgen'),
        ...
        if desi_templates_available:
            cls.cosmics = (os.environ['LVM_ROOT'] +
    ...
    def tearDown(self):
        for expid in [self.expid, 100, 101, 102]:
            ...
            for camera in ('b0', 'r0', 'z0'):
                framefile = lvmspec.io.findfile('frame', self.night, expid, camera=camera)
                if os.path.exists(framefile):
                    os.remove(framefile)
                cframefile = lvmspec.io.findfile('cframe', self.night, expid, camera=camera)
                if os.path.exists(cframefile):
                    os.remove(cframefile)
                skyfile = lvmspec.io.findfile('sky', self.night, expid, camera=camera)
                if os.path.exists(skyfile):
                    os.remove(skyfile)
                fluxcalibfile = lvmspec.io.findfile('calib', self.night, expid, camera=camera)
    ...
    @unittest.skipUnless(desi_root_available, '$LVM_ROOT not set')
    ...
    def test_quickgen_simspec(self):
        ...
        for i in range(len(flavors)):
            ...
            os.environ['LVM_SPECTRO_REDUX'] = os.path.join(os.getenv('LVM_SPECTRO_SIM'), os.getenv('PIXPROD'))
            ...
            if flavor == 'flat':
                ...
                #- verify flat outputs
                fiberflatfile = lvmspec.io.findfile('fiberflat', night, expid, camera)
                ...
                framefile = lvmspec.io.findfile('frame', night, expid, camera)
                self.assertTrue(os.path.exists(framefile))
                cframefile = lvmspec.io.findfile('cframe', night, expid, camera)
