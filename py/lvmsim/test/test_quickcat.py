...
class TestQuickCat(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        isBGS = iibright[0:3]
        isMWS = iibright[3:6]
        targets['LVM_TARGET'][isBGS] = desi_mask.BGS_ANY
        targets['BGS_TARGET'][isBGS] = bgs_mask.BGS_BRIGHT
        targets['LVM_TARGET'][isMWS] = desi_mask.MWS_ANY
        ...
        ii = (targets['LVM_TARGET'] & desi_mask.mask('LRG|ELG|BGS_ANY')) != 0
        truth['TRUESPECTYPE'][ii] = 'GALAXY'
        ii = (targets['LVM_TARGET'] == desi_mask.QSO)
        truth['TRUESPECTYPE'][ii] = 'QSO'
        starmask = desi_mask.mask('MWS_ANY|STD_FSTAR|STD_WD|STD_BRIGHT')
        ii = (targets['LVM_TARGET'] & starmask) != 0
