...
def parse(options=None):
    ...
    if 'LVM_ROOT' in os.environ:
        _default_arcfile = os.path.join(os.getenv('LVM_ROOT'),
    ...
    if args.simspec is None:
        args.simspec = lvmsim.io.findfile('simspec', args.night, args.expid,
                                           outdir=args.outdir)

    if args.fibermap is None:
        #- put in same directory as simspec by default
        filedir = os.path.dirname(os.path.abspath(args.simspec))
        filename = os.path.basename(lvmspec.io.findfile('fibermap', args.night, args.expid))
...
def main(args=None):
    ...
    wave, phot, fibermap = \
        lvmsim.simexp.simarc(arcdata, nspec=args.nspec, nonuniform=args.nonuniform)
    ...
    #- TODO: explain bypassing lvmsim.io.write_simspec
    header = fits.Header()
    ...
    lvmsim.io.write_simspec_arc(args.simspec, wave, phot, header, fibermap, overwrite=args.clobber)
