...
def main(args):
    ...
    if args.load_simz_table is not None:
        ...
        for night in nights:
            for exposure in lvmspec.io.get_exposures(night, raw=True, rawdata_dir=args.rawdir):
                # Ignore exposures with no fibermap, assuming they are calibration data.
                fibermap_path = lvmspec.io.findfile(filetype='fibermap', night=night,
                ...
                for uni_pix in uni_pixels:
                    zbest_files.append(lvmspec.io.findfile('zbest', groupname=uni_pix, nside=64))
