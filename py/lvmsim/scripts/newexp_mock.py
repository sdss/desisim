...
from lvmsim.simexp import simscience, get_mock_spectra
import lvmsim.io
import lvmsim.util
...
def main(args=None):
    ...
    #     obslist['NIGHT'] = [lvmsim.util.dateobs2night(x) for x in obslist['MJD']]
    # else:
    ...
    if args.outdir is None:
        args.outdir = lvmsim.io.simdir(night=night, mkdir=True)
    ...
    telera, teledec = lvmsim.io.get_tile_radec(tileid)
    header = dict(
    ...
    #- Write fibermap to $LVM_SPECTRO_SIM/$PIXPROD not $LVM_SPECTRO_DATA
    fibermap.meta.update(header)
    fibermap.write(lvmsim.io.findfile('simfibermap', night, args.expid,
        outdir=args.outdir), overwrite=args.clobber)

    lvmsim.io.write_simspec(sim, meta, fibermap, obs, args.expid, night, header=header,
        outdir=args.outdir, overwrite=args.clobber)
