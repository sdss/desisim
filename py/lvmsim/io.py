...
from lvmspec.io.util import write_bintable, native_endian, header2wave
import lvmspec.io
import lvmmodel.io

from lvmspec.image import Image
import lvmspec.io.util

from lvmutil.log import get_logger
log = get_logger()

from lvmsim.util import spline_medfilt2d
...
def findfile(filetype, night, expid, camera=None, outdir=None, mkdir=True):
    ...
    Args:
        ...
        outdir (Optional[str]): output directory; defaults to $LVM_SPECTRO_SIM/$PIXPROD
        mkdir (Optional[bool]): create output directory if needed; default True

    Returns:
        str: full file path to output file

    Also see lvmspec.io.findfile() which has equivalent functionality for
    real data files; this function is only be for simulation files.
    """

    #- outdir default = $LVM_SPECTRO_SIM/$PIXPROD/{night}/
    if outdir is None:
        outdir = simdir(night)
...
def write_simspec(sim, truth, fibermap, obs, expid, night, outdir=None, filename=None,
    ...
    lvmutil.depend.add_dependencies(header)
    header['EXPID'] = expid
    header['NIGHT'] = night
...
def write_simspec_arc(filename, wave, phot, header, fibermap, overwrite=False):
    ...
    hx = fits.HDUList()
    hdr = lvmspec.io.util.fitsheader(header)
    hdr['FLAVOR'] = 'arc'
    ...
    for camera in ['b', 'r', 'z']:
        thru = lvmmodel.io.load_throughput(camera)
...
def read_simspec_mpi(filename, comm, spectrographs=None):
    ...
    # is hard-coded several places in lvmsim.  This should be put in lvmmodel
    # somewhere...
...
def write_simpix(outfile, image, camera, meta):
    ...
    meta = lvmspec.io.util.fitsheader(meta)
...
def load_simspec_summary(indir, verbose=False):
    ...
    import astropy.table
    truth = list()
    for fibermapfile in lvmspec.io.iterfiles(indir, 'fibermap'):
...
def find_cosmics(camera, exptime=1000, cosmics_dir=None):
    ...
    Args:
        ...
        cosmics_dir (str, optional): directory to look for cosmics templates; defaults to
            $LVM_COSMICS_TEMPLATES if set or otherwise
            $LVM_ROOT/spectro/templates/cosmics/v0.2  (note HARDCODED version)
    ...
    if cosmics_dir is None:
        if 'LVM_COSMICS_TEMPLATES' in os.environ:
            cosmics_dir = os.environ['LVM_COSMICS_TEMPLATES']
        else:
            cosmics_dir = os.environ['LVM_ROOT']+'/spectro/templates/cosmics/v0.2/'
...
def read_cosmics(filename, expid=1, shape=None, jitter=True):
    ...
    Returns:
        `lvmsim.image.Image` object with attributes pix, ivar, mask
    """
    fx = fits.open(filename)
    imagekeys = list()
...
def find_basis_template(objtype, indir=None):
    """
    Return the most recent template in $LVM_BASIS_TEMPLATE/{objtype}_template*.fits
    """
    if indir is None:
    ...
    if objtype.upper() == 'QSO':
        with fits.open(infile) as fx:
            ...
            if format_version == 1:
                ...
                hdr = fx[0].header
                from lvmspec.io.util import header2wave
...
def simdir(night='', mkdir=False):
    """
    Return $LVM_SPECTRO_SIM/$PIXPROD/{night}
    If mkdir is True, create directory if needed
    """
    dirname = os.path.join(os.getenv('LVM_SPECTRO_SIM'), os.getenv('PIXPROD'), str(night))
...
def empty_star_properties(nstar=1):
    """Initialize a "star_properties" table for lvmsim.templates."""
    from astropy.table import Table, Column
