...
def simulate_frame(night, expid, camera, ccdshape=None, **kwargs):
    ...
    pixfile = lvmspec.io.findfile('pix', night=night, expid=expid, camera=camera)
...
def simulate(camera, simspec, psf, fibers=None, nspec=None, ncpu=None,
    ...
    Args:
        ...
        simspec : lvmspec.io.SimSpec object e.g. from lvmspec.io.read_simspec()
        psf : subclass of specter.psf.psf.PSF, e.g. from lvmmodel.io.load_psf()
        ...
        cosmics (optional): lvmspec.image.Image object from lvmsim.io.read_cosmics()
    ...
    if (comm is None) or (comm.rank == 0):
        ...
        if preproc:
            ...
            image = lvmspec.preproc.preproc(rawpix, header, primary_header=simspec.header)
        else:
