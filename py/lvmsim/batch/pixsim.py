...
NERSC.  This is a temporary pragmatic package -- after lvmspec.pipeline code
...
#- From python
import lvmsim.batch.pixsim
flavors = ['arc', 'flat', 'dark', 'dark', 'gray', 'gray', 'bright', 'bright']
expids = range(len(flavors))
lvmsim.batch.pixsim.batch_newexp('newexp-blat.sh', flavors, expids=expids)
lvmsim.batch.pixsim.batch_pixsim('pixsim-blat.sh', flavors, expids=expids)
...
from lvmsim.batch import calc_nodes
...
def batch_newexp(batchfile, flavors, nspec=5000, night=None, expids=None,
    ...
    if desi_spectro_sim is None:
        if 'LVM_SPECTRO_SIM' in os.environ:
            desi_spectro_sim = os.environ['LVM_SPECTRO_SIM']
        else:
            raise ValueError('must provide desi_spectro_sim or set $LVM_SPECTRO_SIM')
    ...
    with open(batchfile, 'w') as fx:
        ...
        fx.write('export LVM_SPECTRO_SIM={}\n'.format(desi_spectro_sim))
        fx.write('export PIXPROD={}\n'.format(pixprod))
        fx.write('\n')
        fx.write('echo Starting at `date`\n\n')

        fx.write('mkdir -p $LVM_SPECTRO_SIM/$PIXPROD/etc\n')
        fx.write('mkdir -p $LVM_SPECTRO_SIM/$PIXPROD/{}\n'.format(night))
        fx.write('\n')
...
def batch_pixsim(batchfile, flavors, nspec=5000, night=None, expids=None,
    ...
    with open(batchfile, 'w') as fx:
        ...
        for expid in expids:
            infile = lvmspec.io.findfile('raw', night=night, expid=expid, outdir=outdir)
