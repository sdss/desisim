...
import lvmsim
from lvmsim.targets import get_simtype
...
def get_redshift_efficiency(simtype, targets, truth, targets_in_tile, obsconditions=None):
    ...
    if (simtype == 'ELG'):
        ...
        filename = resource_filename('lvmsim', 'data/quickcat_elg_oii_flux_threshold.txt')
        ...
        filename = resource_filename('lvmsim', 'data/quickcat_lrg_rmag_eff.txt')
        ...
        filename = resource_filename('lvmsim', 'data/quickcat_qso_gmag_threshold.txt')
...
def get_observed_redshifts(targets, truth, targets_in_tile, obsconditions):
    ...
    simtype = get_simtype(np.char.strip(truth['TRUESPECTYPE']), targets['LVM_TARGET'], targets['BGS_TARGET'], targets['MWS_TARGET'])
    truez = truth['TRUEZ']
    ...
    for objtype in objtypes:
        if objtype in _sigma_v.keys():
            ...
            if (objtype =='ELG'):
                filename = resource_filename('lvmsim', 'data/quickcat_elg_oii_errz.txt')
