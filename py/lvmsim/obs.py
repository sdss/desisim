...
def new_exposure(program, nspec=5000, night=None, expid=None, tileid=None,
    ...
    lvmsim.targets.sample_objtype() to get the correct distribution of
    ...
    outsimspec = lvmsim.io.findfile('simspec', night, expid)
    outfibermap = lvmsim.io.findfile('simfibermap', night, expid)
    ...
    if program == 'arc':
        if arc_lines_filename is None :
            infile = os.getenv('LVM_ROOT')+'/spectro/templates/calib/v0.4/arc-lines-average-in-vacuum-from-winlight-20170118.fits'
        else :
            infile = arc_lines_filename
        arcdata = fits.getdata(infile, 1)
        if exptime is None:
            exptime = 5
        wave, phot, fibermap = lvmsim.simexp.simarc(arcdata, nspec=nspec, testslit=testslit)

        header['EXPTIME'] = exptime
        lvmsim.io.write_simspec_arc(outsimspec, wave, phot, header, fibermap=fibermap)
        ...
        if flat_spectrum_filename is None :
            infile = os.getenv('LVM_ROOT')+'/spectro/templates/calib/v0.4/flat-3100K-quartz-iodine.fits'
        else :
            infile = flat_spectrum_filename

        if exptime is None:
            exptime = 10
        sim, fibermap = lvmsim.simexp.simflat(infile, nspec=nspec, exptime=exptime, testslit=testslit)

        header['EXPTIME'] = exptime
        header['FLAVOR'] = 'flat'
        lvmsim.io.write_simspec(sim, truth=None, fibermap=fibermap, obs=None,
    ...
    if obsconditions is None:
        ...
        try:
            obsconditions = lvmsim.simexp.reference_conditions[obsconditions.upper()]
        except KeyError:
    ...
    lvmspec.io.write_fibermap(outfibermap, fibermap, header=hdr)
...
def get_next_tileid(program='DARK'):
    ...
    tiles = table.Table(lvmmodel.io.load_tiles())

    #- HACK: update tilelist to include PROGRAM, etc.
    if 'PROGRAM' not in tiles.colnames:
        log.error('You are using an out-of-date desi-tiles.fits file from lvmmodel')
        log.error('please update your copy of lvmmodel/data')
...
def update_obslog(obstype='science', program='DARK', expid=None, dateobs=None,
    ...
    TODO: normalize obstype vs. program; see lvmsim issue #97
    """
