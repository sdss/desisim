...
def match_otype(tbl, objtype):
    ...
    if objtype in ['BGS']:
        targets = (tbl['LVM_TARGET'] & desi_mask['BGS_ANY']) != 0
    elif objtype in ['MWS']:
        targets = (tbl['LVM_TARGET'] & desi_mask['MWS_ANY']) != 0
    elif objtype in ['QSO_L']:
        targets =  match_otype(tbl, 'QSO') & (tbl['TRUEZ'] >= 2.1)
    elif objtype in ['QSO_T']:
        targets =  match_otype(tbl, 'QSO') & (tbl['TRUEZ'] < 2.1)
    else:
        targets = (tbl['LVM_TARGET'] & desi_mask[objtype]) != 0
    # Return
    return targets
