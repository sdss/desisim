...
import lvmmodel.io
from lvmmodel.focalplane import fiber_area_arcsec2
import lvmutil.depend
import lvmspec.interpolation
import lvmsim.io
...
def simarc(arcdata, nspec=5000, nonuniform=False, testslit=False):
    ...
    if testslit:
        ...
        fibermap = astropy.table.Table(lvmspec.io.empty_fibermap(nspec))
...
def simflat(flatfile, nspec=5000, nonuniform=False, exptime=10, testslit=False):
    ...
    wave = lvmspec.io.util.header2wave(hdr)
    ...
    sbflux = lvmspec.interpolation.resample_flux(ww, wave, sbflux)
    wave = ww
...
def _calib_screen_uniformity(theta=None, radius=None):
    ...
    if theta is not None:
        ...
        import lvmmodel.io
        ps = lvmmodel.io.load_platescale()
...
def fibermeta2fibermap(fiberassign, meta):
    ...
    for c in ['FIBER', 'TARGETID', 'LVM_TARGET', 'BGS_TARGET', 'MWS_TARGET',
              'BRICKNAME']:
    ...
    isSTD = (fiberassign['LVM_TARGET'] & stdmask) != 0
    isSKY = (fiberassign['LVM_TARGET'] & desi_mask.SKY) != 0
    isSCI = (~isSTD & ~isSKY)
...
def simulate_spectra(wave, flux, fibermap=None, obsconditions=None,
    ...
    Optional:
        fibermap: table from fiberassign or fibermap; uses X/YFOCAL_DESIGN, TARGETID, LVM_TARGET
...
def get_source_types(fibermap):
    ...
    if 'LVM_TARGET' not in fibermap.dtype.names:
        log.warning("LVM_TARGET not in fibermap table; using source_type='star' for everything")
...
def read_fiberassign(tilefile_or_id, indir=None):
    ...
    if indir is None:
        indir = os.path.join(os.environ['LVM_TARGETS'], 'fiberassign')
...
def testslit_fibermap():
    ...
    fibermap = np.zeros(testslit_nspec, dtype=lvmspec.io.fibermap.fibermap_columns)
...
def read_mock_spectra(truthfile, targetids, mockdir=None):
    ...
    reordered_flux = lvmspec.io.util.native_endian(reordered_flux).astype(np.float64)
