...
class SimSetup(object):
    ...
    def create_surveyfile(self):
        ...
        #rnc
        print(" tile_ids {}".format(self.tile_ids))
    ...
    def simulate_epoch(self, perfect=True, epoch_id=0, truth=None, targets=None, mtl=None, zcat=None):
        ...
        if self.zcat_file is None:
            ...
            newzcat = quickcat(self.tilefiles, targets, truth, zcat=None, perfect=perfect,newversion=True)
            newzcat.write(self.zcat_file, overwrite=True)
        else:
            self.zcat_file = os.path.join(self.tmp_output_path, 'zcat.fits')
            #zcat = Table.read(self.zcat_file, format='fits')
            newzcat = quickcat(self.tilefiles, targets, truth, zcat=zcat, perfect=perfect,newversion=True)
